    ...
    if ($th_screen->id === 'toplevel_page_masterslider') {
        ...
        if ( get_user_meta( $user_id, 'th_ms_install_dismissed', true ) !== '1' ) { ?>
            <div class="notice notice-info">
                <p><?php _e('Looking for the sliders from the live demo? <a href="http://themovation.helpscoutdocs.com/article/198-master-slider-activation" target="_blank">Check out this article for how to import them</a>.', 'bellevue'); ?> <a class="th-dismiss" href="?page=masterslider&th-ms-install-dismissed">Dismiss</a></p>
            </div>
            <?php
...
function themo_admin_meta_show()
{
    ...
    // Admin Scripts
    wp_register_script('themo_admin_js', get_template_directory_uri() . '/assets/js/admin-scripts.js', array(), '1', true);
...
if(!get_option( 'mphb_wizard_passed', false )){
    ...
    $themo_mphb_booking_page = get_page_by_title( 'Booking Confirmation' , OBJECT );
...
/* Kirki Custom Functions (Customizer) */

add_filter( 'kirki_bellevue_theme_dynamic_css', function($styles) {

    if ( function_exists( 'get_theme_mod' ) ) {
        $colors = get_theme_mod( 'themo_mphb_date_colors' );
        $themo_mphb_styling = get_theme_mod( 'themo_mphb_use_theme_styling', true );

        if(isset($colors['mphb_booked_date']) && isset($colors['mphb_available_date']) && isset($themo_mphb_styling) && $themo_mphb_styling == true ){

            $mphb_booked_date = $colors['mphb_booked_date'];
            $mphb_available_date = $colors['mphb_available_date'];

            $custom_css = '.mphb-calendar.mphb-datepick .datepick-month td .mphb-booked-date.mphb-date-check-in {
                            background : linear-gradient(to bottom right, '. $mphb_available_date .' 0, '. $mphb_available_date .' 50%, '.$mphb_booked_date.' 50%, '.$mphb_booked_date.' 100%); 
                        }
                        .mphb-calendar.mphb-datepick .datepick-month td .mphb-booked-date.mphb-date-check-in.mphb-date-check-out {
                            background : linear-gradient(to bottom right, '. $mphb_booked_date .' 0, '. $mphb_booked_date .' 50%, '.$mphb_booked_date.' 50%, '.$mphb_booked_date.' 100%);
                        }
                        .mphb-calendar.mphb-datepick .datepick-month td .mphb-booked-date{
                            background-color: '.$mphb_booked_date.';
                        }
                        .mphb-calendar.mphb-datepick .datepick-month td .mphb-available-date.mphb-date-check-out{
                            background : linear-gradient(to bottom right, '. $mphb_booked_date .' 0, '. $mphb_booked_date .' 50%, '.$mphb_available_date.' 50%, '.$mphb_available_date.' 100%);
                        }
                        .mphb-calendar.mphb-datepick .datepick-month td .mphb-available-date {
                            background-color: '.$mphb_available_date.';
                        }';
            $styles .= $custom_css;
...
if(!function_exists('wdm_add_user_custom_data_options_callback')) {
...
function th_masterslider_update_check() {
    ...
    $th_plugin_dir = WP_PLUGIN_DIR . '/motopress-hotel-booking';
    $th_plugin_slug = 'motopress-hotel-booking';

    if ( is_dir( $th_plugin_dir ) ) {
        // plugin directory found!
        $myUpdateChecker = Puc_v4_Factory::buildUpdateChecker(
            'https://update.themovation.com/hotel-booking.json',
            $th_plugin_dir.'/'.$th_plugin_slug.'.php',
            $th_plugin_slug
        );
    }

    // Hotel Booking WooCommerce Payments add-on
    $th_plugin_dir = WP_PLUGIN_DIR . '/mphb-woocommerce';
    $th_plugin_slug = 'mphb-woocommerce';

    if ( is_dir( $th_plugin_dir ) ) {
        // plugin directory found!
        $myUpdateChecker = Puc_v4_Factory::buildUpdateChecker(
            'https://update.themovation.com/hotel-booking-woocommerce-payments.json',
            $th_plugin_dir.'/'.$th_plugin_slug.'.php',
            $th_plugin_slug
        );
    }
    ...
    $th_plugin_dir = WP_PLUGIN_DIR . '/mphb-request-payment';
    $th_plugin_slug = 'mphb-request-payment';

    if ( is_dir( $th_plugin_dir ) ) {
        // plugin directory found!
        $myUpdateChecker = Puc_v4_Factory::buildUpdateChecker(
            'https://update.themovation.com/hotel-booking-payment-request.json',
            $th_plugin_dir.'/'.$th_plugin_slug.'.php',
            $th_plugin_slug
        );
    }

    // Hotel Booking Reviews add-on
    $th_plugin_dir = WP_PLUGIN_DIR . '/mphb-reviews';
    $th_plugin_slug = 'mphb-reviews';

    if ( is_dir( $th_plugin_dir ) ) {
        // plugin directory found!
        $myUpdateChecker = Puc_v4_Factory::buildUpdateChecker(
            'https://update.themovation.com/hotel-booking-reviews.json',
            $th_plugin_dir.'/'.$th_plugin_slug.'.php',
            $th_plugin_slug
        );
    }
...
function themo_ot_meta_box_post_format_link($array,$pages) {
	...
	$array['fields'] = array(
		...
		array(
			'id'      => '_format_link_url',
			'label'   => '',
			'desc'    => esc_html__( 'Link URL (ex. https://google.com)', 'bellevue' ),
			'std'     => '',
			'type'    => 'text'
		),
		array(
...
function th_register_required_plugins() {
    ...
    // Hotel Booking
    if ( get_theme_mod('themo_tgmpa_hotel_booking',true) ){
    ...
    if ( get_theme_mod('themo_tgmpa_hotel_booking_woocommerce_payments',false) ){
        array_push($plugins, array(
    ...
    if ( get_theme_mod('themo_tgmpa_hotel_booking_payment_request',false) ){
        array_push($plugins, array(
            'name' => 'Hotel Booking Payment Request',
            'slug' => 'mphb-request-payment',
            'source' => 'https://out.themovation.com/mphb-payment-request',
            'required' => false,
        ));
    }

    // Hotel Booking Reviews
    if ( get_theme_mod('themo_tgmpa_hotel_booking_reviews',false) ){
        array_push($plugins, array(
            'name' => 'Hotel Booking Reviews',
            'slug' => 'mphb-reviews',
            'source' => 'https://out.themovation.com/mphb-reviews',
            'required' => false,
        ));
    }
    ...
    if ( get_theme_mod('themo_tgmpa_header_footer',false) ){
        array_push($plugins, array(
